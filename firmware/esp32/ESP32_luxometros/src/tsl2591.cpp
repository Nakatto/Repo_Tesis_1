...
void tsl2591ReadChannels(uint16_t &full, uint16_t &ir) {
    uint16_t fullLow = tsl2591Read8(REG_CHAN0_LOW);
    uint16_t fullHigh = tsl2591Read8(REG_CHAN0_HIGH);
    uint16_t irLow = tsl2591Read8(REG_CHAN1_LOW);
    uint16_t irHigh = tsl2591Read8(REG_CHAN1_HIGH);
...
bool tsl2591Init() {
    Wire.begin();
    uint8_t id = tsl2591Read8(REG_DEVICE_ID);

    // ENABLE = PON | AEN
    tsl2591Write8(REG_ENABLE, 0x03);
    // CONTROL = AGAIN_MED (x25) + ATIME_100MS
    tsl2591Write8(REG_CONTROL, 0x10);
    delay(500);
